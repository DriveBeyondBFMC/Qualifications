...
public:
    ...
    void update(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q, uint64_t delta_time) {
        // newTime = us_ticker_read();
        // deltaT = newTime - oldTime;
        // oldTime = newTime;
        // deltaT = fabs(deltaT * 0.001 * 0.001);
        deltaT = delta_time * 0.001;
        switch (filter_sel) {
