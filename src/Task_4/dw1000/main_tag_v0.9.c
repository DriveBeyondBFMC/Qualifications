...
#define HIGH_SPI_SPEED 10000000

#define UART_DEVICE "/dev/serial0"  // Default UART device on Raspberry Pi
...
// };
dwt_config_t config = {
    5,               // Channel (make sure both devices match)
    DWT_PRF_64M,     // Pulse Repetition Frequency
    DWT_PLEN_128,    // Preamble Length
    DWT_PAC8,        // Preamble Acquisition Chunk
    9,               // TX preamble code
    9,               // RX preamble code
    1,               // Non-standard SFD (0=standard, 1=non-standard)
...
#define POLL_RX_TO_RESP_TX_DLY_UUS 2600  //2600
...
#define RESP_RX_TO_FINAL_TX_DLY_UUS 2800 //2800
/* Receive response timeout */
#define RESP_RX_TIMEOUT_UUS 3500  //2700
...
int main(void)
{
	...
	port_init();
	// UART
	serial_port = serialOpen(UART_DEVICE, BAUD_RATE);
    if (serial_port < 0) {
...
int filter(int input, int fliter_idx )
{
    ...
    if(input > 0)
    {
        ...
        for(int count = 0; count<Filter_D; count++)
        {
...
static void distance_mange(void)
{
    if(GetLocation(&tag_best_solution, 0, &(AnchorList[0]), &(Anthordistance[0])) != -1)
    {
		...
		sprintf(dist_str, "x:%3.1f y:%3.1f",tag_best_solution.x*100,tag_best_solution.y*100);
		serialPuts(serial_port, dist_str);
    }
